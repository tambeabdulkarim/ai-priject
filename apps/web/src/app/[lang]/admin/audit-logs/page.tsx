...
const COPY = {
  ...
  to: 'إلى',
  apply: 'تطبيق التصفية',
  exactNote: 'كل عوامل التصفية أعلاه مطابقة تامة (exact match) في الخادم الحقيقي — وليست بحثًا تقريبيًا.',
    ...
    <div>
      ...
      <div className="ph-form" style={{ flexDirection: 'row', flexWrap: 'wrap', gap: '1rem', alignItems: 'flex-end' }}>
        <div className="ph-field">
          <label className="ph-label" htmlFor="actor">{COPY.actor}</label>
          <input id="actor" className="ph-input" value={draft.actor ?? ''} onChange={(e) => setDraft((d) => ({ ...d, actor: e.target.value || undefined }))} />
        </div>
        <div className="ph-field">
          <label className="ph-label" htmlFor="action">{COPY.action}</label>
          <input id="action" className="ph-input" value={draft.action ?? ''} onChange={(e) => setDraft((d) => ({ ...d, action: e.target.value || undefined }))} />
        </div>
        <div className="ph-field">
          <label className="ph-label" htmlFor="targetType">{COPY.targetType}</label>
          <input id="targetType" className="ph-input" value={draft.target_type ?? ''} onChange={(e) => setDraft((d) => ({ ...d, target_type: e.target.value || undefined }))} />
        </div>
        <div className="ph-field">
          <label className="ph-label" htmlFor="from">{COPY.from}</label>
          <input id="from" type="date" className="ph-input" value={draft.from ?? ''} onChange={(e) => setDraft((d) => ({ ...d, from: e.target.value || undefined }))} />
        </div>
        <div className="ph-field">
          <label className="ph-label" htmlFor="to">{COPY.to}</label>
          <input id="to" type="date" className="ph-input" value={draft.to ?? ''} onChange={(e) => setDraft((d) => ({ ...d, to: e.target.value || undefined }))} />
        </div>
        <button type="button" className="ph-btn-grad" onClick={handleApply}>{COPY.apply}</button>
      </div>
      <p className="ph-page-subtitle">{COPY.exactNote}</p>

      {query.isLoading && <p className="ph-state">{COPY.loading}</p>}
      {query.isError && <p className="ph-state">{getErrorMessage(query.error)}</p>}
      {query.data && items.length === 0 && <p className="ph-state">{COPY.empty}</p>}
        ...
        {items.map((entry) => (
          <div key={entry.id} className="ph-catalogue-card" style={{ cursor: 'default' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              >
                {COPY.details} {expandedId === entry.id ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
              </button>
            </div>
            {expandedId === entry.id && (
              <div style={{ marginTop: '0.75rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div>
                  ...
                  <pre style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>{JSON.stringify(entry.beforeState, null, 2)}</pre>
                </div>
                <div>
                  <strong>{COPY.after}</strong>
                  <pre style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>{JSON.stringify(entry.afterState, null, 2)}</pre>
                </div>
              </div>
            )}
          </div>
      ...
      {query.hasNextPage && (
        <button type="button" className="ph-btn-outline" style={{ marginTop: '1rem' }} onClick={() => query.fetchNextPage()} disabled={query.isFetchingNextPage}>
          {COPY.loadMore}
        </button>
      )}
    </div>
