...
const COPY = {
  ar: {
    ...
    progressBlocked:
      'لا يوجد أي واجهة برمجية في الخادم لعرض عدد المسجّلين أو نسبة الإكمال أو عدد الشهادات الصادرة لكل دورة. هذه ليست قيودًا في الواجهة الأمامية.',
  },
  en: {
    ...
    progressBlocked:
      'No endpoint on the backend exposes per-course enrollment counts, completion percentage, or certificates issued. This is a real backend gap, not a frontend limitation.',
  },
} as const;
      ...
      <main className="ph-page">
        <div className="ph-sec-hdr">
          ...
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            <Link href={withLang(ROUTES.instructorMedia, locale)} className="ph-btn-outline">
              {t.mediaLibrary}
            </Link>
        ...
        <h2 className="ph-catalogue-card-title" style={{ marginTop: '2.5rem' }}>
          {t.yourCourses}
        </h2>
            ...
            {courses?.map((course) => (
              ...
              >
                ...
                <div className="ph-catalogue-card-meta">
                  <StatusBadge status={course.status} locale={locale} />
                </div>
              </Link>
            ))}
          </div>
