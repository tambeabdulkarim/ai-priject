...
const COPY = {
  ar: {
    ...
    activityBlocked: 'يتطلب هذا صلاحية "admin" لعرض سجل التدقيق (audit:read). دور المشرف (moderator) لا يملك هذه الصلاحية في الخادم الحقيقي.',
  ...
  en: {
    ...
    activityBlocked: 'Requires the "admin" permission to view the audit trail (audit:read). The moderator role does not hold this permission on the real backend.',
    loading: 'Loading...',
  ...
  return (
    <div className={locale === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <main className="ph-page">
        ...
        <div className="ph-grid" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))' }}>
        ...
        <Link href={withLang(ROUTES.moderationQueue, locale)} className="ph-btn-grad" style={{ marginTop: '1.5rem', display: 'inline-flex' }}>
          {t.openQueue}
        </Link>
        ...
        <section style={{ marginTop: '2.5rem' }}>
          ...
          {!canViewAudit && <p className="ph-form-error" role="note">{t.activityBlocked}</p>}

          {canViewAudit && auditLogs.isLoading && <p className="ph-state">{t.loading}</p>}
          {canViewAudit && auditLogs.isError && <p className="ph-state">{getErrorMessage(auditLogs.error)}</p>}
          {canViewAudit && auditLogs.data && auditLogs.data.items.length === 0 && <p className="ph-state">{t.empty}</p>}
            ...
            <ul className="ph-form" style={{ gap: '0.5rem' }}>
              {auditLogs.data.items.map((entry) => (
                <li key={entry.id} className="ph-catalogue-card" style={{ cursor: 'default' }}>
                  ...
                  <div className="ph-catalogue-card-meta">{new Date(entry.occurredAt).toLocaleString(locale)}</div>
                </li>
              ))}
            </ul>
          )}
