...
const COPY = {
  ar: {
    ...
    blockedBody: 'الخادم الحقيقي يرفض هذا الطلب دائمًا (HTTP 501) لأن وثيقة تكامل الذكاء الاصطناعي لا تحدد فهرس الميزات، ولا مخطط الإدخال لكل ميزة، ولا مفاتيح الصلاحيات، ولا تطبيقًا فعليًا لمزوّد خارجي. هذا ليس خطأ مؤقتًا ولن يُحل بإعادة المحاولة.',
  ...
  en: {
    ...
    blockedBody: 'The real backend always rejects this request (HTTP 501) because the AI Integration Bible defines no feature catalog, no per-feature input schema, no permission keys, and no real external-provider implementation. This is not a transient error and will not resolve by retrying.',
...
export function BlockedRequestNotice({ error, locale }: BlockedRequestNoticeProps) {
  ...
  const isNotImplemented = error instanceof ApiError && (error.status === 501 || error.code === 'NOT_IMPLEMENTED');
  const isQuotaOrPayment =
    error instanceof ApiError && (error.status === 402 || error.status === 429 || error.code === 'PAYMENT_REQUIRED' || error.code === 'RATE_LIMITED');

  const title = isNotImplemented ? t.blockedTitle : isQuotaOrPayment ? t.quotaTitle : t.genericTitle;
  const body = isNotImplemented ? t.blockedBody : isQuotaOrPayment ? t.quotaBody : getErrorMessage(error);
