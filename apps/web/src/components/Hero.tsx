...
import {
  Search, Rocket, Play,
  Network, BarChart3, Activity,
  TrendingUp, Cpu, PieChart,
  Boxes, Zap, Database, CloudCog,
  Bot, GraduationCap, Map, Users,
  type LucideIcon,
} from 'lucide-react';
...
const LEFT_CARDS: { Icon: LucideIcon; title: string; status: string; kind: 'dot' | 'bar' | 'dots'; pct?: number }[] = [
...
const RIGHT_CARDS: { Icon: LucideIcon; title: string; status: string; value: string; kind: 'bars' | 'progress' | 'ring'; pct?: number }[] = [
  { Icon: TrendingUp, title: 'AI Performance', status: 'كفاءة عالية', value: '98.6%', kind: 'bars' },
  { Icon: Cpu,        title: 'GPU Status',     status: 'NVIDIA A100', value: '92%',  kind: 'progress', pct: 92 },
...
const HERO_STATS: { g: string; Icon: LucideIcon; num: string; lbl: string }[] = [
  { g: 'linear-gradient(135deg,#7c3aed,#a78bfa)', Icon: Bot,           num: '+200', lbl: 'أداة ذكاء اصطناعي' },
  { g: 'linear-gradient(135deg,#db2777,#f472b6)', Icon: GraduationCap, num: '+150', lbl: 'دورة تدريبية' },
    ...
    <section className="ph-hero">
      <div className="ph-hero-inner">
            ...
            {LEFT_CARDS.map((c) => (
              <div key={c.title} className="ph-float-card">
                <div className="ph-float-head">
                  <span className="ph-float-ico"><c.Icon size={14} strokeWidth={2} /></span>
                  <span className="ph-float-title">{c.title}</span>
                </div>
                {c.kind === 'dot' && (
                  <span className="ph-float-status"><i className="ph-dot-live" />{c.status}</span>
                )}
                {c.kind === 'bar' && (
                  <div className="ph-float-bar-row">
                    <div className="ph-float-track"><div className="ph-float-fill" style={{ width: `${c.pct}%` }} /></div>
              ...
              <div key={c.title} className="ph-float-card">
                ...
                {c.kind === 'progress' && (
                  <div className="ph-float-track">
                    <div className="ph-float-fill ph-float-fill-cyan" style={{ width: `${c.pct}%` }} />
                  </div>
                )}
              </div>
            ))}
        ...
        <div className="ph-hero-copy-center">
          <h1 className="ph-h1">تعلم الذكاء الاصطناعي<br />من البداية إلى الاحتراف</h1>
          ...
          <div className="ph-search ph-hero-search">
            <input className="ph-search-input" type="search" placeholder="ابحث عن أداة، دورة، مسار مهني أو موضوع..." aria-label="Search" />
            <button className="ph-search-btn" type="button" aria-label="Search"><Search size={14} strokeWidth={2.5} aria-hidden="true" /></button>
          </div>
          ...
          <div className="ph-hero-cta-row">
            <div className="ph-hero-pill-group" aria-hidden="true">
              {PILLS_LEFT.map((p) => (
                <div key={p.label} className="ph-hero-pill">
                  <span className="ph-hero-pill-ico"><p.Icon size={14} strokeWidth={2} /></span>
                  <div>
            ...
            <div className="ph-hero-btns">
              <a href={`/${locale}/workspace`} className="ph-btn-primary"><Rocket size={16} strokeWidth={2.25} aria-hidden="true" />ابدأ التعلم الآن</a>
              <button className="ph-btn-ghost" type="button"><Play size={16} strokeWidth={2.25} aria-hidden="true" />شاهد كيف تعمل المنصة</button>
            </div>
