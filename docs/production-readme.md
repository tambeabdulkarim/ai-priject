...
- Accounts you will need to create (see `docs/launch-package.md` for the full list): a hosting provider (Vercel + Railway recommended), Neon (or another Postgres provider), Upstash, Backblaze B2 (or another S3-compatible storage provider), Postmark, Stripe, and a domain registrar.
...
```
See `docs/production-secrets-checklist.md` for what each value is used for.
...
### Step 12 — Smoke test
Run through `docs/phase21-launch-preparation.md` Section 5 in full, against your real, live, DNS-resolved URLs — not localhost.
...
`GET /api/v1/health` (public, no auth required — see `apps/api/src/health/health.controller.ts`) returns a JSON object with boolean fields:
...
**Other production endpoints worth confirming respond correctly post-deploy** (a non-exhaustive smoke sample — see `docs/phase21-launch-preparation.md` Section 5 for the full role-by-role checklist):
...
**Smoke-test checklist:** use `docs/phase21-launch-preparation.md` Section 5 in full — it is the authoritative, role-by-role (Learner/Instructor/Moderator/Admin) smoke-test plan and is not duplicated here to avoid the two documents drifting out of sync.

## 4. If something goes wrong

See `docs/deployment-checklist.md`'s Rollback Checklist and `docs/phase21-launch-preparation.md` Section 7 (Disaster Recovery Plan) for per-scenario guidance (database, storage, email, payment, worker, API, frontend failure).
...
- You do not need to build `apps/workers` before launching — it is an empty shell today; Notifications will simply not deliver beyond the synchronous auth-flow emails until it's built (a disclosed, non-blocking gap — see `docs/phase21-v1.1-roadmap.md`).
