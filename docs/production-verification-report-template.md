...
*(paste the complete console output of `node scripts/production-smoke-test.js` here, unedited)*
...
**This is a local pre-deployment validation run, performed to confirm the smoke-test tooling itself works correctly. It is not a production deployment and must not be read as one — no production infrastructure exists yet (see `docs/phase21-launch-preparation.md`).**
...
| Database healthy | ⚠️ WARN → ✅ PASS on retry | First check reported `database: false` (a Neon cold-start delay, a previously-documented, known behavior — see `docs/phase16-production-validation-report.md`); reconnected and reported `true` seconds later. Not a defect. |
