...
**Created:** Phase 21 (Production Infrastructure Preparation), 2026-08-06 · Preparation only — this checklist is meant to be executed at actual deployment time, not during this phase. Cross-references: `docs/production-secrets-checklist.md` (what each variable is), `.env.production.example` / `.env.server.production.example` (the templates these fill in), `docs/phase19-execution-readiness-report.md` (the original Owner Action List this checklist formalizes into a printable form).

## Pre-Deployment

- [ ] Confirm Phoenix v1.0 is on the frozen, intended commit (per `docs/version-1.0-freeze.md`).
...
- [ ] Read `docs/production-secrets-checklist.md` in full — know every value you'll need before starting.
- [ ] Decide launch scope explicitly: is AI in scope for day one? Is the async Notifications worker required, or launching without it (degrades gracefully — see `docs/phase21-v1.1-roadmap.md`)?

## Infrastructure

- [ ] Choose hosting provider(s) — Vercel (frontend) + Railway (backend/workers) recommended per `docs/phase17-deployment-launch-guide.md`'s comparison; confirm or override.
...
- [ ] See `docs/phase21-launch-preparation.md` Section 6 for the full monitoring plan this checklist item summarizes.
...
## Deployment Order (summary — see `docs/phase21-launch-preparation.md` Section 4 for full detail)
...
- [ ] Run the full smoke-test checklist in `docs/phase21-launch-preparation.md` Section 5 against the real, live, DNS-resolved production URLs.
...
- [ ] See `docs/phase21-launch-preparation.md` Section 7 for the full disaster-recovery plan (per-scenario: database, media, email, payment, worker, API, frontend failure).
