...
- [ ] HTTPS enforcement confirmed (HTTP requests redirect) once DNS is live — verify with `../04-Smoke-Test/production-smoke-test.js` (requires the full Phoenix repository beside this package — see this document's Prerequisites)'s Security category.

## Vercel (frontend hosting — recommended, see the main Phoenix repository's `docs/phase17-deployment-launch-guide.md`'s comparison)
- [ ] Account created.
...
- [ ] (Optional, once built) a second Railway service for `apps/workers`, once the Notifications-delivery worker exists (the main Phoenix repository's `docs/phase21-v1.1-roadmap.md`) — not required for initial launch.
...
## Neon (database — recommended, see the main Phoenix repository's `docs/phase17-deployment-launch-guide.md`)
...
- [ ] One real object write+read confirmed (e.g. via `../04-Smoke-Test/production-smoke-test.js --deep`'s Storage category — requires the full Phoenix repository beside this package, see `../03-Handover-Guide/handover-guide.md`'s Prerequisites).
...
- [ ] `JWT_PRIVATE_KEY`/`JWT_PUBLIC_KEY`/`PASSWORD_PEPPER`/`MFA_ENCRYPTION_KEY` freshly generated on the owner's own machine (never inside an AI session — see `../06-Environment-Variables/production-secrets-checklist.md`).
...
## Security Headers
- [ ] Confirmed present on the live production backend: CSP, X-Content-Type-Options, X-Frame-Options, Strict-Transport-Security (all built Phase 16) — verify with `../04-Smoke-Test/production-smoke-test.js` (requires the full Phoenix repository beside this package — see this document's Prerequisites)'s Security category against the real production URL, not just localhost.
...
## Recovery
- [ ] `../02-Production-Checklist/deployment-checklist.md`'s Rollback Checklist and `../08-Deployment-Order-And-Rollback/phase21-launch-preparation.md`'s Disaster Recovery Plan reviewed by whoever is executing the deployment, before it begins — not read for the first time during an incident.
